...
Copied from: https://github.com/emilsvennesson/kodi-viaplay/blob/master/resources/lib/viaplay.py
"""
import os
...
class Viaplay(object):
    def __init__(self, settings_folder, country, debug=False):
        ...
        self.base_url = 'https://content.viaplay.{0}/{1}'.format(self.domain, self.device_key)
        self.login_api = 'https://login.viaplay.%s/api' % self.domain
        try:
    ...
    def get_stream(self, guid, pincode=None, tve='false'):
        ...
        stream = {}
        url = 'https://play.viaplay.%s/api/stream/bymediaguid' % self.domain
        params = {
            'deviceId': self.get_deviceid(),
            'deviceName': 'web',
            'deviceType': 'pc',
